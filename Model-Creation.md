...
AWS Recipe
Remove features related to loan origination and status… we’re only interested in predicting the grade
Addr_state + zip_code - Combine state and zip code because the zip code is masked it may not be as meaningful as we’d like… could combine with state
